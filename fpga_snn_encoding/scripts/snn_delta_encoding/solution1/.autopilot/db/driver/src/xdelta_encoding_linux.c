...
typedef struct {
    ...
    XDelta_encoding_uio_map maps[ MAX_UIO_MAPS ];
} XDelta_encoding_uio_info;

/***************** Variable Definitions **************************************/
static XDelta_encoding_uio_info uio_info;
...
static int uio_info_read_name(XDelta_encoding_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDelta_encoding_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDelta_encoding_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDelta_encoding_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDelta_encoding_Initialize(XDelta_encoding *InstancePtr, const char* InstanceName) {
	XDelta_encoding_uio_info *InfoPtr = &uio_info;
