...
__attribute__((sdx_kernel("delta_encoding", 0))) void delta_encoding(
    pixel_t img[(28 * 28)],
    spike_t spikes[25][(28 * 28)],
    float threshold,
    float leak
...
uint32_t count_spikes(spike_t spikes[25][(28 * 28)]);
# 7 "../src/encoding.cpp" 2
...
#pragma HLS INTERFACE m_axi depth=(28 * 28) port=img offset=slave
#pragma HLS INTERFACE m_axi depth=25*(28 * 28) port=spikes offset=slave
#pragma HLS INTERFACE s_axilite port=seed

 lfsr_state_global = (uint32_t)seed;

    TIME_LOOP: for (int t = 0; t < 25; t++) {
        PIXEL_LOOP: for (int i = 0; i < (28 * 28); i++) {
#pragma HLS PIPELINE II=1

 float rand_val = lfsr_rand_float();
            float intensity = img[i];
...
#pragma HLS INTERFACE s_axilite port=seed
 ...
 INIT_T: for (int t = 0; t < 25; t++) {
        INIT_I: for (int i = 0; i < (28 * 28); i++) {
#pragma HLS PIPELINE II=1
 spikes[t][i] = 0;
        }
    }
...
#pragma HLS PIPELINE II=3
 ...
 float intensity = img[i];
        ...
        float mu = (1.0f - intensity) * (float)(25 - 1);
        ...
        if (spike_time >= 25) spike_time = 25 - 1;
...
#line 79 "E:/1_Research/Neuromorphic/NEUROMORPHIC/First_research/fpga_snn_encoding/scripts/run_hls_encodingterpisah.tcl"
#pragma HLSDIRECTIVE TOP name=delta_encoding
# 120 "../src/encoding.cpp"
...
#pragma HLS INLINE off
 ...
 uint32_t count = 0;
    ...
    VITIS_LOOP_167_1: for (int t = 0; t < 25; t++) {
        VITIS_LOOP_168_2: for (int i = 0; i < (28 * 28); i++) {
#pragma HLS PIPELINE II=1
