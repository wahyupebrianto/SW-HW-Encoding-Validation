...
typedef struct {
    ...
    XLatency_encoding_uio_map maps[ MAX_UIO_MAPS ];
} XLatency_encoding_uio_info;

/***************** Variable Definitions **************************************/
static XLatency_encoding_uio_info uio_info;
...
static int uio_info_read_name(XLatency_encoding_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XLatency_encoding_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XLatency_encoding_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XLatency_encoding_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XLatency_encoding_Initialize(XLatency_encoding *InstancePtr, const char* InstanceName) {
	XLatency_encoding_uio_info *InfoPtr = &uio_info;
