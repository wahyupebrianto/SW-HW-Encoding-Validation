...
int main() {
    cout << "=======================================" << endl;
    cout << "LATENCY ENCODING TEST - 10K IMAGES" << endl;
    cout << "=======================================" << endl;
    ...
    for (int idx = 0; idx < NUM_TEST_IMAGES; idx++) {
        ...
        latency_encoding(img, spikes, seed);
    ...
    cout << "\n=======================================" << endl;
    cout << "LATENCY ENCODING RESULTS" << endl;
    cout << "=======================================" << endl;
    printf("Sparsity:    %.2f%%\n", avg_sparsity);
    printf("Norm Rate:   %.4f\n", avg_norm);
    
    // Validation real results in software
    const float SW_SPARSITY = 96.00;
    const float SW_NORM = 0.0400;
    ...
    if (err_sp < 1.0f && err_nr < 0.001f) {
        cout << "\n✓ PASS - statistically consistent (deterministic)" << endl; //Temporal distribution differs due to RNG
    } else {
        cout << "\n⚠ WARNING - Should be exact!" << endl;
    }
